...
QuadDynamics::QuadDynamics(string name) /* (ISay-0016) trace */
 : BaseDynamics(name) 
{ /* (ISay-0018) trace */
  Initialize();
}
...
{
  if(!BaseDynamics::Initialize()) return 0; /* (ISay-0020) just as called returns one - no condition*/

  _initialized = false;
	
    /* (ISay-0021) Loading all the drone parameters: */
  ...
  _flightMode = config->Get(_name+".SimMode", "Full3D");
    ...
    /* (ISay-0022) State is: xyz-uvw-phithetapsi-pqr (Not sure about the [SEEME:] coordinate systems
    ). It is set as empty - I suppose - in the base class */
  ResetState(V3F());
  ...
  /* (ISay-0023) a big story. Skip it for now. But overall, its just loading parameters for
  controller object. Note that if the [SEEME:] controllerType is set rather than 'QuadControl', 
  'controller' will be still empty */
  ...
  /* (ISay-0024) Setting initial states based on defined parameters */
  ...
  /* (ISay-0025) [DOME] A lot to tell about delegates ... */
...
{ /* (ISay-0035) performing an update for one time-step */
	...
	while(remainingTimeToSimulate > 0.000001) // Time intervals lower than that are just discarded (for speed of running)
	{
		...
		{
			/* (ISay-0036) Simulate INS sensors */
      		// generate virtual gyro and accelerometer data
			V3F newRawGyro = V3F(omega+sqrtf(gyroNoiseInt/dt)*V3F(gasdev(idum),gasdev(idum),
				gasdev(idum)));
			...
			/*(ISay-0037) Generation of real-states (I suppose) - controller command - motor commands*/
			// This is the update of the onboard controller -- runs timeout logic, sensor filtering, 
			// estimation, controller, and produces a new set of motor commands
		...
		const double simStep = MIN(controllerUpdateInterval - timeSinceLastControllerUpdate, 
			remainingTimeToSimulate);
		/* (ISay-0038) And finally this is simulation of quadrotor plant */
		Dynamics(simStep, simulationTime, externalForceInGlobalFrame, externalMomentInBodyFrame, 
			idum);
...
void QuadDynamics::Dynamics(float dt, float simTime, V3F external_force, V3F external_moment, 
  	int& idum)
{ /* (ISay-0039) This is all of the quadrotor dynamics as simulated in this software. Read it,
	check it with theories and see what type of equation set did they use, and compare it with 
	other versions */
	...
	for (int i = 0; i < 4; i++)
	{
		curCmd.desiredThrustsN[i] = CONSTRAIN(curCmd.desiredThrustsN[i], minMotorThrust, 
			maxMotorThrust);
		motorCmdsN(i) = curCmd.desiredThrustsN[i] + randomMotorForceMag * 
			ran1_inRange(-1.f, 1.f, idum);
	}
